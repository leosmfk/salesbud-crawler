...
test("usa a data em horário de Brasília, não em UTC", () => {
  ...
  expect(transcriptFileName(meeting())).toBe("2026-08-20_Rennova_<>_Strattum_2679290.txt");
});

test("não deixa espaço nenhum no nome", () => {
  const nomes = [
    transcriptFileName(meeting()),
    transcriptFileName(meeting({ title: "[Salesbud + Strattum] Follow-Up!" })),
    transcriptFileName(meeting({ title: "  espaços   duplicados  " })),
  ];
  for (const nome of nomes) expect(nome).not.toInclude(" ");
});

test("colapsa underscores em vez de empilhar", () => {
  expect(transcriptFileName(meeting({ title: "a   b" }))).toBe("2026-08-20_a_b_2679290.txt");
});
